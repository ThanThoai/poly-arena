...
from schemas import (
    BOBotStats, BOCreate, BOForecastStats, BOResponse, DualModeResponse,
...
def _read_ws_snapshot(
    ...
    sr = get_sync_redis()
    for key in (
        f"{ORDERBOOK_KEY_PREFIX}:{symbol}:{timeframe}:{direction}:{candle_open}",
...
def _try_walk_asks(
    ...
    """Try to fill a MARKET BUY by walking asks.

    Returns (avg_price, num_shares, walk_levels) if successful, None if
    the orderbook cannot satisfy the order (slippage exceeded, no liquidity, FOK rejected).
    """
    slippage = slippage_tolerance if slippage_tolerance is not None else _DEFAULT_SLIPPAGE_TOLERANCE
    best_ask = Decimal(str(asks[0][0]))
    max_price = best_ask * (Decimal("1") + Decimal(str(slippage)))
    ...
    if ceiling_price is not None:
        max_price = min(max_price, Decimal(str(ceiling_price)))
...
def _refund_dual_balance(bot: Bot, amount: float, is_dual: bool) -> None:
    """Refund amount to both REST and WS pools (if dual) and sync legacy balance."""
    if bot.balance_rest is not None:
        bot.balance_rest = round(bot.balance_rest + amount, 8)
    if is_dual and bot.balance_ws is not None:
        bot.balance_ws = round(bot.balance_ws + amount, 8)
    bot.balance = bot.balance_rest if bot.balance_rest is not None else round(bot.balance + amount, 8)


@router.post("", response_model=DualModeResponse, status_code=201)
def create_bo(
    payload: BOCreate,
    ...
    cost_with_fee = payload.amount + max_fee

    # Dual-balance pools (REST + WS) for MARKET orders
    _br = bot.balance_rest if bot.balance_rest is not None else bot.balance
    _bw = bot.balance_ws if bot.balance_ws is not None else bot.balance
    can_rest = _br >= cost_with_fee
    can_ws = _bw >= cost_with_fee
    is_dual_market = not is_limit and (can_rest and can_ws)

    if not can_rest and not can_ws:
        raise HTTPException(status_code=400, detail="Insufficient balance (including estimated fee)")
    # For LIMIT orders, use legacy balance check
    if is_limit and bot.balance < cost_with_fee:
        raise HTTPException(status_code=400, detail="Insufficient balance (including estimated fee)")

    # Deduct from REST pool
    if can_rest:
        bot.balance_rest = round(_br - payload.amount, 8)
    # Deduct from WS pool (only for dual MARKET)
    if is_dual_market:
        bot.balance_ws = round(_bw - payload.amount, 8)
    # Keep legacy balance synced to REST pool
    bot.balance = bot.balance_rest if bot.balance_rest is not None else round(bot.balance - payload.amount, 8)
    ...
    if is_limit and is_aggressive_limit:
        ...
        if is_partial:
            ...
            if bot.balance_rest is not None:
                bot.balance_rest = round(bot.balance_rest + refund, 8)
        ...
        if is_dual_market:
            ...
            try:
                ...
                if ws_is_partial:
                    ...
                    bot.balance_ws = round(bot.balance_ws + ws_refund, 8)
                    ws_amount = round(ws_actual_cost, 8)

                # Taker fee on WS fill
                ws_entry_fee = taker_fee_from_levels(ws_walk_levels)
                if ws_entry_fee > 0:
            ...
            if ws_reject_reason is None:
                ...
                if ws_reason:
                    ws_reason = f"{ws_reason} | {ws_timing_info}"
                else:
                    ws_reason = ws_timing_info

                ws_traces = [make_trace(
                    "MATCHING", "WS_FILL",
                    f"WS MARKET filled from snapshot arriving {ws_latency_ms:.1f}ms after order. "
                    f"Avg: ${ws_avg_price:.4f}. Shares: {ws_num_shares:.4f}. "
                    f"Fee: ${ws_entry_fee:.4f}.",
                ...
                ws_bo = BinaryOption(
                    ...
                    original_amount   = original_market_amount,
                    result            = BOResult.PENDING,
                    avg_price         = ws_avg_price,
                    num_shares        = ws_num_shares,
                    reason            = ws_reason,
                    order_received_at = order_received_at,
                    ask_fetched_at    = ask_fetched_at,
                    settlement_at     = settlement_at,
                    ...
                    session_id        = session_id,
                    candle_open       = candle_open,
                    entry_fee         = ws_entry_fee,
                    order_type        = payload.order_type,
                    ceiling_price     = payload.ceiling_price,
                    me_order_status   = None,
                    walk_prices       = {"entry": ws_walk_levels},
                    traces            = ws_traces,
                    fill_source       = "WS",
                    pair_id           = bo.id,
                )
    ...
    logger.info(
        "BO #%d CREATED: fill_source=%s me_status=%s shares=%s avg_price=%s amount=%.4f",
        bo.id, bo.fill_source, bo.me_order_status,
        f"{bo.num_shares:.4f}" if bo.num_shares else "0",
        f"{bo.avg_price:.6f}" if bo.avg_price else "N/A",
        bo.amount,
    )

    # Build dual-mode response
    rest_resp = BOResponse.model_validate(bo)
    ws_resp = None
    if ws_bo is not None:
        ws_bo.snapshot_best_ask      = snapshot_prices["best_ask"]
        ws_bo.snapshot_best_ask_size = snapshot_prices["best_ask_size"]
        ws_bo.snapshot_best_bid      = snapshot_prices["best_bid"]
        ws_bo.snapshot_best_bid_size = snapshot_prices["best_bid_size"]
        ws_resp = BOResponse.model_validate(ws_bo)

    return DualModeResponse(rest_order=rest_resp, ws_order=ws_resp)
...
def list_bo(
    ...
    fill_source: Optional[str]          = Query(None, description="Filter by fill source: REST or WS"),
    ...
    if fill_source:
        q = q.filter(BinaryOption.fill_source == fill_source.upper())
    total = q.count()
